...
public class DateUtils {
    ...
    public static String getDistanceTime(LocalDateTime one, LocalDateTime two) {
        ...
        long time1 = one.atZone(ZoneId.systemDefault()).toEpochSecond();
        long time2 = two.atZone(ZoneId.systemDefault()).toEpochSecond();
        long diff ;
        if(time1<time2) {
            diff = time2 - time1;
        } else {
    ...
    public static String getYearAndMothen(LocalDate date,String pattern) {

        return date.format(DateTimeFormatter.ofPattern(pattern));
    }
    public static String getYearAndMothen(LocalDate date) {

        return date.format(DateTimeFormatter.ofPattern("yyyyMM"));
    }
    public static String getDateAndMinute(LocalDateTime date) {
     ...
     * @return
     */
    public static String fromToday(LocalDateTime localDateTime) {
...
/*        Date firstDate = new Date(FIRST);
    ...
    public static String getFullDate(LocalDateTime createDate) {
        SimpleDateFormat simple = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        return simple.format(createDate);
    ...
    public static LocalDateTime yyyyMM(String dateString){
        DateTimeFormatter formatter = new DateTimeFormatterBuilder()
                .appendPattern("yyyyMMdd")
                .parseDefaulting(ChronoField.DAY_OF_MONTH, 0)
                ...
                .parseDefaulting(ChronoField.SECOND_OF_MINUTE, 0)
                .toFormatter();
   //     System.out.println("____________"+dateString);
        LocalDateTime getBegym_开始年月 = LocalDateTime.from(formatter.parse(dateString+"01"));
        return getBegym_开始年月;
    }
}